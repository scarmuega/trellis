...
(schema in the spec's `runtime.md`). The `metrics/actuals/` freshness window
...
| derivation sweep | weekly  | org/steward | on founding-map/strategy/contract edits: escalate downstream artifacts and consumer contexts for revalidation; flag orphaned subdomains and economically orphaned strategies; for decisions accepted since the last sweep, confirm the standing guidance landed in its governing artifact (spec rule 6) — report unreachable decisions to their owner as orphaned operative content; a pre-existing pile gets one report naming the compaction procedure, never one per decision |
...
| archive sweep    | monthly | org/steward | file terminal artifacts colder than the retention horizon into `archive/` (`trellis.toml` → `archive.after_days`); a move, never a verdict — nothing non-terminal is filed and no status changes |
